...
/** EXAMPLE */
const MINTING_CONFIGURATION = {
    NETWORK: 250,
    NFT_NAME: "Creatures Of The Cave 2",
    SYMBOL: "COTC2",
    MAX_PER_TX: 5,
    MAX_SUPPLY: 4000,
...
export const MintProject: React.FC<MintProjectProps> = ({
    ...
    const decreaseMintAmount = () => {
        const newMintAmount = Math.max(mintAmount - 1, 1);
        ...
        const init = async () => {
            ...
            try {
                await mintingContext.init({
                    contractAddress,
                    abi,
                    gasLimit: MINTING_CONFIGURATION.GAS_LIMIT,
                    weiCost: MINTING_CONFIGURATION.WEI_COST,
    ...
    return (
        <div>
            <div className="flex">
                <MintingContainer style={{ width: '50%', margin: '0 auto' }}>
                    <MintHeader className="mb-4">{baseInformation.name}</MintHeader>
                    <div className="mb-4">
                        <img className="rounded-md" style={{ maxWidth: 400, margin: '0 auto' }} src='/images/StrangePigs.gif' />
                    </div>

                    <MintTooling className="p-4">
                        <ProgressBar min={0} height={10} max={baseInformation.maxSupply} value={mintingContext.mintCount} />
                        ...
                        <div className="flex items-center justify-center mt-4">
                            <div className="mr-8">
                                <Button small={true} onClick={decreaseMintAmount}>-</Button>
                                <span className="mx-4">{mintAmount}</span>
                                <Button small={true} onClick={increaseMintAmount}>+</Button>
                            </div>

                            <Button variant="filled" onClick={handleMintClick}>Mint</Button>
                        </div>

                        <div>Price in total: {weiToDisplayCost((baseInformation?.mint?.weiCost as number) * mintAmount, NETWORKS.find(item => item.symbol === 'FTM') as Network)} </div>
                    </MintTooling>
                </MintingContainer>
